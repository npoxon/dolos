    ...
    for<'a> UtxoOutputModelBuilder<'a>: IntoModel<T, SortKey = (u64, usize, u32)>,
{
    ...
    // heights are monotonic in slots, so the range is applied on slots
...
///
/// 1. The slot of every creating tx comes from the archive index, no block
///    body is read. Rows are sorted by slot, which fixes the order between
///    blocks and lets the page window be located.
    ...
    for<'a> UtxoOutputModelBuilder<'a>: IntoModel<T, SortKey = (u64, usize, u32)>,
{
    ...
    let block_deps = block_meta
        .resolve_batch(window.iter().map(|(_, txo_ref)| txo_ref.0))
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
